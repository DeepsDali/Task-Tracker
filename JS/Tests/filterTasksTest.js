export const filterTasksTest = () => {
    ...
    const tasks = [...taskList.childNodes].filter(parent => parent.tagName == "DIV");
    const taskCount = tasks.length;
    completedBtn.click();
    let visibleTasks = tasks.filter(task => (task.style.display == "flex")).length;
    equal(
        visibleTasks,
        0,
    ...
    equal(
        visibleTasks,
        taskCount - 1,
        `When one tasks checked off pending filter should have all tasks minus one visible. Expected ${taskCount - 1}, received ${visibleTasks}`
    )
    allBtn.click();
    visibleTasks = tasks.filter(task => (task.style.display == "flex")).length;
    equal(
    ...
    equal(
        visibleTasks,
        taskCount - 1,
        `When one task set to work type home filter should have all but 1 tasks visible. Expected ${taskCount - 1}, received ${visibleTasks}`
    )
    workBtn.click();
    visibleTasks = tasks.filter(task => (task.style.display == "flex")).length;
    equal(
